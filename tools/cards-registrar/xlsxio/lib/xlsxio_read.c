...
uint32_t ZCALLBACK minizip_io_filehandle_read_file_fn (voidpf opaque, voidpf stream, void* buf, uint32_t size)
{
  ssize_t len;
...
long ZCALLBACK minizip_io_filehandle_seek_file_fn (voidpf opaque, voidpf stream, uint32_t offset, int origin)
{
  int whence;
  if (!opaque || !stream)
    return -1;
...
uint32_t ZCALLBACK minizip_io_memory_read_file_fn (voidpf opaque, voidpf stream, void* buf, uint32_t size)
{
  uLong len;
  if (!opaque || !stream || !buf || size == 0)
    return 0;
...
long ZCALLBACK minizip_io_memory_seek_file_fn (voidpf opaque, voidpf stream, uint32_t offset, int origin)
{
  switch (origin) {
    case ZLIB_FILEFUNC_SEEK_CUR :
      /*if (offset < 0) {
