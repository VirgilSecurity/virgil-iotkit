    ...
    uint16_t ref_result_size)
{
    ...
    BOOL_CHECK_RET(VS_CODE_OK == secmodule_impl->hash(hash_type, data, data_sz, result_buf, sizeof(result_buf), &result_sz),
    ...
    uint16_t correct_result_size)
{
    ...
    BOOL_CHECK_RET(VS_CODE_OK == secmodule_impl->hash(hash_type, (uint8_t*)test_data, VS_IOT_STRLEN(test_data), result_buf, sizeof(result_buf), &result_sz),
        "Error execute hash op");
    ...
    BOOL_CHECK_RET(VS_CODE_OK == secmodule_impl->hash(hash_type, (uint8_t*)another_test_data, VS_IOT_STRLEN(another_test_data), another_result_buf, sizeof(another_result_buf), &result_sz),
        "Error execute hash op");
    BOOL_CHECK_RET(result_sz == correct_result_size, "Incorrect size of result");
