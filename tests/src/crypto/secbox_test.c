...
/******************************************************************************/
static bool
_test_case_secbox_save_load(vs_storage_op_ctx_t *ctx, const char *filename,  vs_secbox_type_t type, const char *test_data, uint32_t data_sz) {
...
_test_case_secbox_del(vs_storage_op_ctx_t *ctx, const char *filename) {
    ...
    BOOL_CHECK_RET(_test_case_secbox_save_load(ctx, filename, VS_SECBOX_SIGNED, buf, strlen(buf)), "Error create file for delete test")
...
vs_secbox_test(vs_storage_op_ctx_t *ctx) {
    ...
    TEST_CASE_OK(
            ...
            _test_case_secbox_save_load(ctx, TEST_FILENAME_LITTLE_DATA, VS_SECBOX_SIGNED, _little_test_data, sizeof(_little_test_data)))
    ...
    TEST_CASE_OK(
            "Read/write small piece of data. Signed and encrypted",
            _test_case_secbox_save_load(ctx, TEST_FILENAME_LITTLE_DATA, VS_SECBOX_SIGNED_AND_ENCRYPTED, _little_test_data, sizeof(_little_test_data)))

    TEST_CASE_OK("Read/write big piece of data.Signed and encrypted",
                 _test_case_secbox_save_load(ctx, TEST_FILENAME_BIG_DATA, VS_SECBOX_SIGNED_AND_ENCRYPTED, _big_test_data, big_test_size))
terminate:;
    if (_big_test_data) {
