...
_test_hmac_case(vs_hsm_hash_type hash_type, const uint8_t *correct, uint16_t correct_sz){
    ...
    VS_HSM_CHECK_RET(vs_hsm_hmac(hash_type, key_raw, sizeof(key_raw), input_raw, sizeof(input_raw), buf, sizeof(buf), &sz),
...
test_hmac(void) {
    ...
    TEST_CASE_OK("SHA-256 usage", _test_hmac_case(VS_HASH_SHA_256, sha256_result_raw, sizeof(sha256_result_raw)));
    TEST_CASE_OK("SHA-384 usage", _test_hmac_case(VS_HASH_SHA_384, sha384_result_raw, sizeof(sha384_result_raw)));
    TEST_CASE_OK("SHA-512 usage", _test_hmac_case(VS_HASH_SHA_512, sha512_result_raw, sizeof(sha512_result_raw)));
